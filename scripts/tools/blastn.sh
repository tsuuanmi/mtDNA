...
# =============================================================================
# BLASTn Tool Script — Single-command wrapper around src/tools/blastn/pipeline.py
#
# Runs BLASTN batch processing (AB1 → BLASTN TSV → ETL → Sample JSON).
...
usage() {
    ...
    echo ""
    echo "Run BLASTN batch processing for a single batch."
    echo ""
    echo "Options:"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        -h|--help)
            ...
            ;;
        -s|--step)
            # Ignored — kept for pipeline.sh -s compatibility
            shift 2
            ;;
        --data-dir)
            DATA_DIR="$2"
...
CMD=(
    uv run python -m src.tools.blastn.pipeline
    --input-dir "$DATA_DIR"
...
echo "[blastn] Running BLASTN pipeline for ${BATCH_ID}"
echo "[blastn]   data-dir:    ${DATA_DIR}"
echo "[blastn]   sample-list: ${SAMPLE_LIST}"
echo "[blastn]   output-dir:  ${OUTPUT_DIR}"
echo "[blastn]   ref-path:    ${REF_PATH}"
if [ -n "$JSON_DIR" ]; then
    echo "[blastn]   json-dir:    ${JSON_DIR}"
fi
