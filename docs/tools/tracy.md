...
├── pipeline.py        # batch orchestration, parallel ETL, CLI entry point
├── preprocessing.py   # AB1 -> Tracy decompose (external subprocess)
├── quality_control.py  # planned: trace metrics, windows, noisy ranges, QC report
...
batch — it reads sample IDs, calls `preprocessing.decompose_sample()` per
sample, runs `etl.process()` in parallel via `ProcessPoolExecutor`, combines
same-LID reads, and writes the `json/`, `regions/`, and `preprocess/` trees.
`preprocessing.py` is the only module that invokes the external Tracy binary,
and it is called only by `pipeline.py`. `quality_control.py` is planned as the
single source of truth for trace metrics, 10–20-base window analysis, noisy
range detection, and QC report records. `transforms.py` and `utils.py` are pure
helper libraries used by `etl.py`; `utils.py` has no dependency on
`Sample`/`Variant`. Data flows `preprocessing.decompose_sample()` ->
`etl.process()` -> `Sample` -> `Batch` (`gen_seq_from_variants_regions()`,
...
preprocess directory. The function returns that directory path (created with
`mkdir(parents=True, exist_ok=True)`), even when no AB1 files are found or the
binary is missing, so callers can detect an empty run. Subprocess failures
(non-zero return code) are logged with their `stderr` and the batch continues.
...
2. **Load JSON** (`_load_tracy_json`) and **normalize `ref1pos`**
   (`_normalize_ref1pos`) — Tracy >= 0.7.8 emits `ref1pos` as a single integer
...
3. **Detect primer type** (`detect_primer_type`) — derive `PrimerTypeInfo` from
   `input_path.stem`.
4. **Compute trim bounds** (`_compute_trim_bounds`) — measure leading/trailing
   dash counts in `alt1align`/`ref1align`; `trim_left`/`trim_right` are the max
   alt/ref dash counts at each edge; derive inclusive `start_index`/`end_index`;
   keep `ref_end_dash` for reverse-strand math.
...
Each sample is processed in a `ProcessPoolExecutor` worker via
`_process_single_sample`, which builds a `DecomposeConfig` from settings, calls
`decompose_sample(...)` into `preprocess_dir/<sample_id>/`, and then runs
`etl.process()` for every `*.json` whose name contains `"HV"`. The planned QC
step will analyze each decompose JSON after this step and write one batch QC
report before noise ranges are used for future variant masking. QC problems
must be recorded and must not stop the rest of the batch. Per-sample failures
(`ValueError`, `KeyError`, `OSError`, `RuntimeError`) are logged and the batch
continues; each successful JSON yields a `{"sample_id", "sample"}` result.
...
└── preprocess/                       # Tracy artifacts and planned QC report
    ├── qc_report.json                # planned trace noise QC report
    └── <sample_id>/                  # one subdir per sample (decompose_sample output)
        ├── <stem>.align
        ├── <stem>.decomp
        ├── <stem>.bcf
...
The planned `preprocess/qc_report.json` will describe every Tracy decompose
JSON, including clean, suspicious, likely-noisy, unavailable, and error
results. It will contain the window settings, thresholds, per-trace metrics,
and merged reference-coordinate ranges. The first QC version will report these
ranges without changing variant output.
...
parameters (`trim`, `pratio`, `maxindel`, `quality_threshold`,
`min_peak_value`, `heteroplasmy_threshold`, `max_workers`) are **not** CLI
flags — they come from `TracySettings` / the `MTDNA_TRACY_*` env vars.
...
- **`pipeline.process_batch(input_dir, output_dir, options=None) -> dict[str, Sample]`** — batch orchestration: decompose + ETL per sample in parallel, combine same-LID reads, write `json/` + `regions/` + `preprocess/`.
- **`pipeline.BatchOptions`** — dataclass (`ref_path`, `batch_id`, `samples_path`) for `process_batch()`.
- **`pipeline.main() -> None`** — CLI entry point.
- **`preprocessing.decompose_sample(sample, input_dir, output_dir, ref_path, *, config=None) -> Path`** — run the external `tracy decompose` on a sample's AB1 files; returns the per-sample preprocess directory.
...
Module communication is one-directional: `pipeline.py` imports from `etl.py`,
`preprocessing.py`, and `utils.py`; `etl.py` imports from `transforms.py`,
`utils.py`, and `src/core` (`flagging`, `models`); `etl.py` never imports from
`pipeline.py`, `utils.py` never depends on `Sample`/`Variant`, and the CLI
wrapper holds no business logic.
