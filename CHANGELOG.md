## Unreleased
### Added
- **statistics**: Add approved-export population analysis with callable denominators from declared intervals and normalized variants, exact complete three-HV and regional haplotypes, rare-variant summaries, empirical match probability/discrimination/diversity, Wilson intervals, optional reference annotations, auditable JSON/TSV and executive reports.
- **statistics**: Add `mtDNA_population_data_issues.tsv` to population reports: one row per sample, region and issue for fixable upstream representation problems — `missing_intervals`, `duplicated_or_overlapping_intervals` (the offending pairs) and `unverifiable_variant_representation` (each entry with the reference base it contradicts) — with affected-sample counts in the summary JSON and executive summary so the upstream records can be found and corrected.
- **regenerate**: Add `region_table.json`, a final-ready view written after final batch JSON generation. It preserves every sample field and canonical variant dict, changing only the variant grouping from type to HV region.
...
- **statistics**: The merger now writes `<results>/merged/sample_discrepancies.tsv`: one row per unique sample ID across the merged TSV and JSON with `discrepancies` flags (`duplicate_rows`, `in_tsv_not_json`, `in_json_not_tsv`, `matched`; flags can combine), plus the merged-TSV row count and batches and the JSON batch — making duplicate samples and one-sided TSV/JSON sample IDs trackable as data instead of console logs; flag counts equal the console-log counts.
...
### Changed
- **lab**: Swap the default checker and approver signature names in generated LAB reports.
- **lab**: Move LAB tracking/rerun workbooks and generated TSV to ignored `data/modules/LAB/`; `scripts/modules/LAB/run.sh` now rebuilds `tracking.tsv` from `tracking.xlsx`, reads the current `rerun.xlsx` layout without the expected OpenPyXL data-validation warning, writes reports under `temp/modules/LAB/`, reads batch metadata from `${DATA_DIR}/metadata/`, normalizes date-based tracking batch IDs while preserving exact MS batch matches, discovers FASTA batches in both active and archive result roots, and skips only batches with both completed HTML and Excel reports so interrupted runs resume safely.
...
- **statistics**: Move and rename the data/results merger to `src/modules/statistics/merge.py`; run it with `python -m src.modules.statistics.merge`.
- **statistics**: Restructure the data/results merger into the `src/modules/statistics/merge/` package (concern-separated modules mirroring `population/`); the CLI (`python -m src.modules.statistics.merge`) is unchanged, and merged outputs are byte-identical. Directory scans use one Python implementation (the `find` subprocess path was removed), and the scan cache is persisted once per scan.
- **statistics**: Make the merger's NAS metadata ingestion fast: metadata TSVs are read concurrently (32 workers), cached per source file in `scan_cache.json` (schema version 3, also bypassed by `--force-refresh`), and Excel workbooks are re-converted only when their batch TSV is missing or unreadable — removing the duplicate full read of every metadata TSV per run (cold full-cache-rebuild run ~3 min → ~5 s; warm repeat run ~4 s → ~1.4 s).
- **statistics**: The merger takes its input and output locations as CLI arguments like the population module: `--data-dir`, `--results-dir` and `--output-dir` (defaults: the production NAS roots and `<results-dir>/merged`), replacing the implicit `.env` `DATA_DIR`/`RESULTS_DIR` constants; callers can pass `.env` values explicitly, path-dependent API functions take a frozen `MergePaths` contract, importing the package no longer requires environment variables, and env-config validation was removed. Merged outputs are byte-identical.
- **statistics**: Make approved-cohort population analysis about 11x faster on the current 90k-sample export (full run ~6.5 min to ~35 s) with byte-identical reports: identical plain profiles are grouped and computed once with member multiplicity, callability derives from interval range arithmetic instead of per-position checks, and validated export entries are model-checked in one bulk pass.
...
- **core/flagging**: Remove the dead `recompute_no_315_1_flag` helper and its `TestRecomputeNo3151Flag` tests. It was superseded by the generalized `recompute_sample_flags`, which recomputes the "No 315.1 variant" absence flag — and all other core sample-level flags — at JSON-write time; the module docstring and `docs/core/flagging.md` now reference `recompute_sample_flags`.
- **statistics**: Remove the obsolete `scripts/modules/statistics/run.sh` wrapper; run the merger directly with `python -m src.modules.statistics.merge`.
### Fixed
...
- **statistics**: Fix `src/modules/statistics/query_multi_filters.py` executing its full query run at import time — loading `merged_statistics.json` from the NAS and rewriting the seven query TSVs (e.g. `NBAA0316.tsv`) into the current directory — so every pytest run that imports all `src/` modules regenerated those stray files in the workspace root. The run logic is now wrapped in `main()` behind an `if __name__ == "__main__"` guard; running the module directly is unchanged.
### Added
